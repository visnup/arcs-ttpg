...
await Promise.all([
  cp(src("card-data/arcs/en-US"), ".", { recursive: true }),
  cp(src("faq/arcs/en-US.yml"), "faq.yml"),
  cp(src("errata/arcs/en-US.yml"), "errata.yml"),
]);

const base = parse(await readFile("arcsbasegame.yml", "utf8"));
const leaders = parse(await readFile("leaders-lore.yml", "utf8"));
const campaign = parse(await readFile("blightedreach.yml", "utf8"));
// const errata = parse(await readFile("errata.yml", "utf8"));
const faq = new Map(
  parse(await readFile("faq.yml", "utf8")).map(({ card, faq }) => [
    card,
    faq
      .map(({ q, a }) => `Q: ${q}\nA: ${a}\n`)
      .join("\n")
